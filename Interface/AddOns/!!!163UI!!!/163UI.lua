...
local safecall,copy,tinsertdata,tremovedata = safecall,copy,tinsertdata,tremovedata
local U2, _, U1 = 163, ...
local L = U1.L;
...
local function dbLoaded(db)
    db.prevVersion = db.version or -1;
    db.version = tonumber(GetAddOnMetadata("!!!163UI!!!","X-163UI-Version")or"0")
end
...
local function getInitialAddonInfo()
    ...
    for i = 1, GetNumAddOns() do
        ...
        title = title:gsub("%|cff880303%[网易有爱%]%|r ", ""):gsub("%|cff880303%[有爱%]%|r ", "")
...
function U1SaveDBValue(cfg, value)
    ...
    if cfg.dontCompareDefaultWhenSave then
        U1DBA.configs[cfg._path] = U1EncodeNIL(value);
        return old
    end
    ...
    if type(value) == "table" then
        if has_default and type(default) == "table" and tcovers(value, default) and tcovers(default, value) then
            U1DBA.configs[cfg._path] = nil
        else
            U1DBA.configs[cfg._path] = value
        end
...
function U1UpdateTags(onlyAffectLoaded, addonName)
    ...
    --处理ALL,NETEASE,SINGLE三个含有filter的标签
    --hide的不加入tagInfo
...
function U1RegisterAddon(name, infoReg)
    ...
    if infoReg.tags ~= nil then
        for k, v in next, infoReg.tags do
            infoReg.tags[k] = _G[v] or v;
            if not strfind(v, "^TAG") or _G[v] == nil then
                print(name, v);
            end
        end
    end
...
local function startCapturing()
    ...
    --处理AceEvent的情况，有个问题是，初始加载的版本必须比后面的大  --  当前版本不存在这个问题
...
function U1:ToggleAccountGlobalSetting(v, use_current)
    if v then
        if U1DBG.__useAccountGlobalSettingBlackList == nil then
            U1DBG.__useAccountGlobalSettingBlackList = {  };
        end
        if U1DBG.__useAccountGlobalSettingChars == nil then
            U1DBG.__useAccountGlobalSettingChars = {  };
        end
        U1DBG.__useAccountGlobalSetting = true;
        if use_current then
            U1DBG.__globalSetting = U1DB;
            U1DBG.__useAccountGlobalSettingChars[UnitGUID('player')] = true;
        end
        ReloadUI();
    else
        U1DBG.__useAccountGlobalSetting = nil;
        if use_current then
            U1DB = deepmix({}, U1DBG.__globalSetting);
        end
    end
end

function U1:ApplyGlobalSetting()
    U1DBG.__useAccountGlobalSettingChars[UnitGUID('player')] = true;
    U1DBG.__useAccountGlobalSettingBlackList[UnitGUID('player')] = false;
    local _global = U1DBG.__globalSetting;
    for addon, state in next, _global.addons do
...
StaticPopupDialogs['U1INITIALIZE_GLOBAL_FOR_CHAR'] = {preferredIndex = 3,
	text = L["Reload to apply addon account global setting?"],
	button1 = YES,
	button2 = NO,
    OnAccept = function(self, data)
        U1:ApplyGlobalSetting();
    end,
	hideOnEscape = 1,
	timeout = 0,
	exclusive = 1,
...
function U1:ADDON_LOADED(event, name)
    if name == _ then
        ...
        if U1DBG.__useAccountGlobalSetting and not U1DBG.__useAccountGlobalSettingBlackList[UnitGUID('player')] then
            if U1DBG.__globalSetting ~= nil then
                if U1DBG.__useAccountGlobalSettingChars[UnitGUID('player')] == nil then
                    db = defaultDB;
                    U1DBG.__useAccountGlobalSettingBlackList[UnitGUID('player')] = true;
                        ...
                        INIT_COMPLETED = function()
                            ...
                            ticker = C_Timer.NewTicker(1.0, function()
                                if InCinematic() then
                                    ...
                                    C_Timer.After(2.0, function() StaticPopup_Show("U1INITIALIZE_GLOBAL_FOR_CHAR", nil, nil, {  }); end);
                                    ticker:Cancel();
                                end
                            end);
                        end
                    ...
                    db = U1DBG.__globalSetting;
                end
            else
                db = defaultDB;
                U1DBG.__globalSetting = db;
            end
        else
        ...
        if U1.returnFromDisableAll then
            --恢复之前的状态
            for k, v in pairs(U1DBA and U1DBA.addons or {}) do
        ...
        hooksecurefunc("EnableAddOn",  function(name, charName)
            if charName == nil or charName == true or charName == U1PlayerName then
                saveState(name, 1)
            end
        end)
        hooksecurefunc("DisableAddOn", function(name, charName)
            if charName == nil or charName == true or charName == U1PlayerName then
                saveState(name, 0)
            end
        end)
        ...
        if(not IsAddOnLoaded(name)) then
            ...
            if InCombatLockdown() then --and info.secure then --5.0 script ran too long
                ...
                if not secureLoadAnnounced and secure then
                    ...
                    U1OutputAddonLoaded(name, loaded, reason);
                    local speed = loadSpeed2 or U1DBA.loadSpeed or 20
...
function U1:PLAYER_ENTERING_WORLD(event)
    --有时VARIABLES_LOADED会在ENTERING_WORLD之后
...
do
    function U1FramePosGet(name)
        U1DBA.frames = U1DBA.frames or {};
        U1DBA.frames[name] = U1DBA.frames[name] or {};
        return U1DBA.frames[name];
    end
...
function U1:PLAYER_LOGOUT(event)
    ...
    if(not self.PROFILE_CHANGED) then
        --最后再保存一次位置
        for k, v in pairs(U1DBA.frames) do
...
function U1:LOADING_SCREEN_DISABLED()
    _B_LoadingScreen = false;
end
function U1:LOADING_SCREEN_ENABLED()
    _B_LoadingScreen = true;
end
function CoreIsLoadingScreen()
