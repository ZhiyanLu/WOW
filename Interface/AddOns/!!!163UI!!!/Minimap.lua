...
function U1MMB_CheckMinimapChildren()
    ...
    for i=1, select("#", Minimap:GetChildren()) do
        ...
        if not btn or btn=="MinimapBackdrop" or btn:IsProtected() or btn.__u1mmbchecked then
            ...
            if(name and name:sub(1,10)=="HandyNotes") then
                --HandyNotes卡顿
...
function U1MMB_CreateContainer()
    ...
    UUI.MakeMove(ct)
    ct:SetFrameStrata("MEDIUM")
    hooksecurefunc(UUI(), "SetFrameStrata", function() ct:SetFrameStrata("MEDIUM") end)
...
function U1_MMBCreateCoordsButton()
    local btn = CreateFrame("Frame","MinimapCoordsButton",UIParent,PatchTemplate())
