...
function UUI.SetAddonTooltip(addonName, tip)
    ...
    if(info.dummy) then
        tip:AddLine(L["网易有爱插件集"], 0, 0.82, 0);
        return
    end
    ...
    if(not info.vendor)then
        ...
        tip:AddLine(L["网易有爱整合版"], 0, 0.82, 0)
    end
end
...
function UUI.Top.Create(main)
    ...
    :un()
    CoreUIEnableTooltip(main.setting, L["有爱设置"], L["直接显示网易有爱的介绍和配置项，再次点击则恢复当前的选择插件"])
    ...
    do
        ...
        local soundSlider = TplSlider(main.setting.soundPanel, nil, VOLUME, 1, "%d%%", 0, 100, 5):Key("soundSlider"):Size(12,128):TOP(-10, -35)
        :SetScript("OnEnter", UIDropDownMenu_StopCounting):SetScript("OnLeave", UIDropDownMenu_StartCounting)
...
function UUI.Center.Create(main)
    ...
    StaticPopupDialogs["163UIUI_LOAD_ALL_CONFIRM"] = {preferredIndex = 3,
        text = "您确认吗？不建议您加载全部插件\n\n网易有爱整合了非常多的优秀插件，但玩家一般用不到全部功能。建议您在默认方案的基础上选择几个自己需要的，不然会显得有点杂乱。",
        button1 = YES,
        button2 = CANCEL,
    ...
    local btn = TplPanelButton(center,nil, UUI.PANEL_BUTTON_HEIGHT):Key("loadAll"):Set3Fonts(UUI.FONT_PANEL_BUTTON)
    :SetScript("OnClick", function()
        local tag = U1GetSelectedTag()
        if not tag or tag == "NETEASE" or tag == "BIG" then
    ...
    local btn = TplPanelButton(center,nil, UUI.PANEL_BUTTON_HEIGHT):Key("disableAll"):Set3Fonts(UUI.FONT_PANEL_BUTTON)
...
function UUI:DB_LOADED()
    local main = UUI();
    if U1DBA.lastSearch then
        main.search:SetSearchText(U1DBA.lastSearch)
    end
    main.center.cols = U1DBA.cols or UUI.CENTER_COLS;
...
function UUI.CreateUI()
    ...
    --缩放按钮
    main:Texture(nil, nil, "Interface\\BUTTONS\\UI-AutoCastableOverlay", 0.619, 0.760, 0.612, 0.762):Key("resizeButtonBG"):Size(14):BR(UUI.BORDER_WIDTH-2, -UUI.BORDER_WIDTH+2)
    ...
    CoreUIEnableTooltip(search, L["搜索插件及选项"], function(self, tip)
        ...
        tip:AddLine(" ");
        tip:AddLine(L["只有网易有爱官方支持的插件才能用拼音搜索名称。"], 0,0.82,0,true);
    end)
    ...
    :un()
    CoreUIEnableTooltip(GameMenuFrame.btn163, L["网易有爱"], L["点击有爱标志开启插件控制中心\n \nCtrl点击小地图图标可以收集/还原"])
...
function U1_CreateMinimapButton()
    ...
    local ldb = LibStub("LibDataBroker-1.1"):NewDataObject(U1Name, {
        ...
        OnTooltipShow = function(tip)
            tip:AddLine(L["网易有爱插件中心"])
            tip:AddLine(L["    网易有爱（163UI）是网易大神隆重推出的新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，网易有爱实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"], nil, nil, nil, 1)
            tip:AddLine(" ")
    ...
    U1DBA.minimapPos = U1DBA.minimapPos or 191
    LibStub("LibDBIcon-1.0"):Register("U1MMB", ldb, U1DBA);
    LibStub("LibDBIcon-1.0"):Show("U1MMB");
...
if DEBUG_MODE then
    ...
    if not UUI.FIRST then
        ...
        WW:Run(function(name)
            U1_FRAME_NAME = name;
            UUI.CreateUI()
        --if U1DBA then U1DBA.lastSearch = nil end
