...
StaticPopupDialogs['U1PROFILE_SAVE_CURRENT_TO_GLOBAL'] = { preferredIndex = 3,
	text = L["SAVE CURRENT PROFILE AS ACCOUNT GLOBAL PROFILE?"],
	button1 = YES,
    button2 = CANCEL,
    button3 = NO,
	OnAccept = function(self, data)
        U1:ToggleAccountGlobalSetting(true, true);
    end,
    OnAlt = function()
        U1:ToggleAccountGlobalSetting(true, false);
    end,
    OnCancel = function()
    end,
	hideOnEscape = 1,
...
StaticPopupDialogs['U1PROFILE_RESET_TO_CHAR_OR_GLOBAL'] = { preferredIndex = 3,
	text = L["USE GLOBAL PROFILE OR CHARACTER'S OLD PROFILE?"],
    button1 = L["GLOBAL PROFILE"],
    button2 = CANCEL,
	button3 = L["CHARACTER PROFILE"],
    OnAccept = function(self, data)
        U1:ToggleAccountGlobalSetting(false, true);
    end,
    OnAlt = function()
        U1:ToggleAccountGlobalSetting(false, false);
    end,
    OnCancel = function()
    end,
	hideOnEscape = 1,
...
StaticPopupDialogs['U1PROFILE_SIMPLESTYPE'] = {preferredIndex = 3,
	text = L["Do you want to use Simple UI?"],
	button1 = YES,
	button2 = NO,
    OnAccept = function(self, data)
        _163_Preset_Simple();
...
f.TitleText:SetText(L["NetEaseUI Profiles"])
...
f.btnGlobalSetting = TplPanelButton(f):SetText(L["Global Setting"]):AutoWidth() f.btnReset:SetWidth(f.btnReset:GetWidth()+4)
CoreUIEnableTooltip(f.btnGlobalSetting, L["Global Setting"],
    L["Global Setting Tip"] .. "\n" ..
    "\124cffffffff" .. L["Global Setting"] .. (U1DBG.__useAccountGlobalSetting and
        ": \124cff00ff00已开启\124r" .. "\n" ..
        "\124cffffffff" .. L["Global Setting Character"] .. (U1DBG.__useAccountGlobalSettingBlackList and U1DBG.__useAccountGlobalSettingBlackList[UnitGUID('player')] and ": \124cffff0000正在使用角色自己的独立配置\124r" or ": \124cff00ff00正在使用帐号通用配置\124r")
    or
        ": \124cffff0000已关闭\124r"
    )
)
f.btnSimple = TplPanelButton(f):SetText(L["Simple Style"]):AutoWidth() f.btnSimple:SetWidth(f.btnSimple:GetWidth()+4)

CoreUIAnchor(f.InsetBg, "BOTTOMRIGHT", "TOPRIGHT", -5, 0, "RIGHT", "LEFT", 0, 0, --[[f.btnImport,]] f.btnNew, f.btnReset, f.btnGlobalSetting, f.btnSimple)
...
f.btnGlobalSetting:RegisterForClicks("AnyUp");
f.btnGlobalSetting:SetScript("OnClick", function(self, button)
    if button == "LeftButton" then
        if U1DBG.__useAccountGlobalSetting then
            StaticPopup_Show("U1PROFILE_RESET_TO_CHAR_OR_GLOBAL", nil, nil, {  });
        else
            StaticPopup_Show("U1PROFILE_SAVE_CURRENT_TO_GLOBAL", nil, nil, {  });
        end
    elseif button == "RightButton" then
        if U1DBG.__useAccountGlobalSetting then
            if U1DBG.__useAccountGlobalSettingBlackList[UnitGUID('player')] then
                StaticPopup_Show("U1INITIALIZE_GLOBAL_FOR_CHAR", nil, nil, {  });
            else
...
f.btnSimple:SetScript("OnClick", function(self)
    StaticPopup_Show'U1PROFILE_SIMPLESTYPE'
end);
