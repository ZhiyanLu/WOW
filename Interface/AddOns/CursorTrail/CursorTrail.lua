...
kGameTocVersion = select(4, GetBuildInfo()) or 80200
...
Globals.SlashCmdList[kAddonName] = function (params)
    local usageMsg = kAddonName.." "..kAddonVersion.." Commands:\n"
                ..BLUE.."  /ct combat"..GREEN.." - Toggles the 'Show only in combat' setting.\n"
                ..BLUE.."  /ct fade"..GREEN.." - Toggles the 'Fade out when idle' setting.\n"
                ..BLUE.."  /ct mouselook"..GREEN.." - Toggles the 'Show during Mouse Look' setting.\n"
                ..BLUE.."  /ct reload"..GREEN.." - Reloads the current cursor settings.\n"
                ..BLUE.."  /ct reset"..GREEN.." - Reset cursor to original settings.\n"
                .."|r Profile Commands:\n"
...
function PlayerConfig_Validate()
    ...
    if ( not PlayerConfig.ModelID 
         or (kGameTocVersion >= 80200 and not tonumber(PlayerConfig.ModelID)) 
       ) then
        PlayerConfig.ModelID = kDefaultModelID
    end
    
    -- Clear obsolete fields.
    if (kGameTocVersion >= 80200 and PlayerConfig.BaseScale ~= nil) then
...
function CursorModel_Load(config)
    ...
    if (kGameTocVersion >= 80200) then
        ...
        CursorModel:SetModel("spells\\lightningboltivus_missile.mdx")
        CursorModel.Constants = CopyTable(kDefaultModelID)
    end
