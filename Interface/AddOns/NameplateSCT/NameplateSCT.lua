...
function NameplateSCT:DamageEvent(guid, spellID, amount, school, crit, spellName)
    ...
    if (icon ~= "only") then
		...
		elseif (self.db.global.truncate and amount >= 10000) then
            text = string.format("%.1f", amount / 10000);
...
function NameplateSCT:MissEvent(guid, spellID, missType)
    ...
    text = MISS_EVENT_STRINGS[missType] or "Miss";
...
local fontFlags = {
    ...
    ["THICKOUTLINE"] = "细描边",
    ["nil, MONOCHROME"] = "单色",
    ["OUTLINE , MONOCHROME"] = "单色描边",
    ["THICKOUTLINE , MONOCHROME"] = "单色细描边",
};
